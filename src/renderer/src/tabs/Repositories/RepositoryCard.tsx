...
export const RepositoryCard: React.FC<RepositoryCardProps> = ({ repo, onClick }) => {
	...
	return (
		<div className="group relative">
			<button type="button" onClick={() => onClick(repo)} className="w-full cursor-pointer">
				<Card
					className={cn(
						'relative w-full border p-4 transition-all',
						currentSession
			...
			{!currentSession && (
				<a
					href={repo.html_url}
					className="absolute right-3 top-3 text-zinc-400 transition-all hover:text-zinc-900 group-hover:translate-x-1"
					onClick={(event) => {
						event.preventDefault()
						event.stopPropagation()
						window.electron.openExternal(repo.html_url)
					}}
				>
